...
contract = w3.eth.contract(address="0x150E8c1BB5064BbE8D511643a94486b7dE988753", abi=abi)
    ...
    if command and "enroll" in command:
        ...
        if user_name:
            ...
            if speaker_profiles:
                ...
                speak("Account Created Successfully With Address.")
                        ...
                        if command:
                            ...
                            # Enrollment of contact
                                ...
                                if contract.functions.isEnrolled(name).call():
                                    ...
                                    speak("Creating the address of the contact")
                            ...
                            # getAddress
                                ...
                                name=listen_for_command()
                                if(contract.functions.isEnrolled(name).call()):
                                    address=contract.functions.getAddressByName(name).call()
                            ...
                            # Transfer
                            ...
                            elif "send ether" in command:
                                speak("Please Enter The Name of the Person to Transfer Ether")
                                name=listen_for_command()
                                if(contract.functions.isEnrolled(name).call()):
                                    recipient_address=contract.functions.getAddressByName(name).call()
                                    speak("Please State The Amount To Transfer")
                                ...
                                receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
                                speak(f"Transferred {amount} ether to {recipient_address}. Transaction confirmed in block {receipt.blockNumber}.")
                                print(f"Transferred {amount} ether to {recipient_address}. Transaction confirmed in block {receipt.blockNumber}.")
                            ...
                            elif "list commands" or "help" in command:
                                speak("List of commands : time, enroll, exit, unenroll, is enroll, get address, balance, transfer, deposit, withdraw, list commands")
                                print("List of commands : time, enroll, exit, unenroll, is enroll, get address, balance, transfer, deposit, withdraw, list commands")
